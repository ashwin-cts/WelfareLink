...
=======
    ...
    public async Task<(bool isEligible, string errorMessage)> ValidateGenderEligibilityAsync(int programId, string? citizenGender)
    {
        ...
        if (string.IsNullOrEmpty(citizenGender))
        {
            return (false, "Please update your profile with your gender before applying for this program.");
        }

        if (citizenGender != eligibleGender)
        {
            return (false, $"This program is only open to {eligibleGender} applicants.");
        }
